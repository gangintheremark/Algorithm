...
5x5 크기의 알파벳 격자에서 상하좌우/대각선으로 인접한 칸들의 글자를 이어서 단어를 찾아내는 것이다. <br>
`완전 탐색`을 이용해, 단어를 찾아낼 때까지 모든 인접한 칸을 하나씩 시도해 보는 것이다. 그중 한 칸에서라도 단어를 찾을 수 있으면 성공이고, 어느 칸을 선택하더라도 답이 없다면 실패. <br>
다음과 같은 형태의 함수를 작성한다.
<br>
